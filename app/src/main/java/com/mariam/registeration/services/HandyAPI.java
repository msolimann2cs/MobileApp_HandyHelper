package com.mariam.registeration.services;

public class HandyAPI {
    private String mariam_api = "192.168.1.8:3000";
    private String seif_api = "192.168.100.8:3000";
    private String salma_api = "192.168.1.5:3000";
    private String BYOD_api="192.168.0.26:3000";
    private String soliman_api = "10.39.1.162:3000";
    private String home_server = "10.39.1.39:3000";
    private String auc_api = "10.40.39.215:3000";
    public String API_LINK = auc_api;

}
